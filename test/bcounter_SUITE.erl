...
greater_insert_basic(_Config) ->
  ...
  [V1, V2] = read_keys("BCGreater", "1", ["bcA, bcB"]),
  ?assertEqual(2, V1),
  ?assertEqual(11, V2).
...
greater_udpate_basic(_Config) ->
  ...
  [V1, V2] = read_keys("BCGreater", "20", ["bcA", "bcB"]),
  ?assertEqual(3, V1),
  ?assertEqual(15, V2),
  % decrement
  ...
  [V3, V4] = read_keys("BCGreater", "20", ["bcA", "bcB"]),
  ?assertEqual(1, V3),
  ?assertEqual(11, V4).
...
greater_update_fail(_Config) ->
  ...
  [V1, V2] = read_keys("BCGreater", "30", ["bcA", "bcB"]),
...
smaller_insert_basic(_Config) ->
  ...
  [V1, V2] = read_keys("BCSmaller", "1", ["bcA", "bcB"]),
  ?assertEqual(4, V1),
  ?assertEqual(14, V2).
...
smaller_update_basic(_Config) ->
  ...
  [V1, V2] = read_keys("BCSmaller", "20", ["bcA", "bcB"]),
  ?assertEqual(4, V1),
  ?assertEqual(14, V2),
  % decrement
  ...
  [V3, V4] = read_keys("BCSmaller", "20", ["bcA", "bcB"]),
  ?assertEqual(2, V3),
  ?assertEqual(10, V4).
...
smaller_update_fail(_Config) ->
  ...
  [V1, V2] = read_keys("BCSmaller", "30", ["bcA", "bcB"]),
  ?assertEqual(3, V1), % assert value does not change on fail
  ?assertEqual(13, V2),
  ?assertThrow(_, tutils:aql("UPDATE BCSmaller SET bcA INCREMENT 3 WHERE ID = 30")),
  ?assertThrow(_, tutils:aql("UPDATE BCSmaller SET bcB INCREMENT 3 WHERE ID = 30")),
  [V3, V4] = read_keys("BCSmaller", "30", ["bcA", "bcB"]),
